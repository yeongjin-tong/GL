...
public class CircuitGraph : MonoBehaviour
{
    ...
    /// <summary>
    /// ✨ [요청 1. 신규 추가]
    /// 씬에 존재하는 모든 '유효한' 와이어를 기반으로 넷 그래프 전체를 다시 빌드합니다.
    /// 와이어가 삭제되었을 때 호출되어야 합니다.
    /// </summary>
    public void RebuildGraph()
    {
        ...
        // 2. 씬에 있는 모든 와이어를 찾습니다.
        Wire[] allWires = FindObjectsOfType<Wire>();

        foreach (var wire in allWires)
        {
            // 3. '유효한' 와이어(이전에 구현한 IsValid 함수 사용)에 대해서만
            if (wire != null && wire.IsValid())
            {
                // 4. 연결을 다시 등록합니다.
                RegisterConnection(wire.connectedPoints[0].parentComponent, wire.connectedPoints[1].parentComponent);
            }
        }
    }
}