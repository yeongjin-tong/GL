...
public class SymbolPaletteController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (currentPlacingPrefab != null)
        {
            ...
            // 2. 변환된 로컬 좌표를 그리드에 맞게 스냅합니다.
            Vector2 snappedPosition = localPoint; // 기본값은 마우스 위치
            if (gridManager != null)
            {
                // GridManager가 있다면, 계산된 그리드 위치를 사용합니다.
                snappedPosition = gridManager.SnapToGrid(localPoint);
            }
